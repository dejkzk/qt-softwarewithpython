...
//    QChart_hall->setTheme(QtCharts::QChart::ChartThemeDark);
    ...
    PythonInit();//python脚本调用初始化

//    //调用脚本处理信号
//     QTextStream cout(stdout,  QIODevice::WriteOnly);
//     Py_SetPythonHome(L"D:/SYS ruanjian/python38/");
//     Py_Initialize();
//        if ( !Py_IsInitialized() )
//        {
//         qDebug()<<-1;
...
//        //PyRun_SimpleString( "filter1 = hello.filter1" );
//        //PyRun_SimpleString( "filter1()" );
//        cout << "point1" << endl;

//        //调用脚本处理信号
//         //QTextStream cout(stdout,  QIODevice::WriteOnly);
//         PyObject * pFunc = NULL;PyObject * pModule = NULL;
//         PyObject *pReturn = NULL;PyObject *pDict = NULL;
//   //      Py_SetPythonHome(L"D:/SYS ruanjian/python38/");
//   //      Py_Initialize();
//   //         if ( !Py_IsInitialized() )
//   //         {
...
//            pModule = PyImport_ImportModule("hello");// 这里的hello是创建的python文件************记得修改*************
...
//                int a = (int)qrand()%200 + 1065;
//                PyList_SetItem(PyList,i, PyFloat_FromDouble(a));//给PyList对象的每个元素赋值

//            }
//            PyTuple_SetItem(ArgList, 0, PyList);//将PyList对象放入PyTuple对象中
//            //PyTuple_SetItem(ArgList, 1, Py_BuildValue("i",5));//将PyList对象放入ArgList中
//            pReturn=PyObject_CallObject(pFunc, ArgList);//调用函数，完成传递
//            qDebug()<<pReturn; //输出返回值
//            cout<<"list size:"<< PyList_Size(pReturn)<<endl;
...
//             }
//            free(pReturn);//释放P所指向的内存空间,但指针变量p仍然留在栈中,成为了野指针
//            pReturn = NULL;//释放野指针
...
void MyWidget::tcp_data_trans_together_func(QByteArray data_in_array)//高八位低八位数据整合
{
    ...
    for (i=0;i<40;i++)
    {
        data_return[4*i] = data_buff[i*16+1]+data_buff[i*16+2]+data_buff[i*16+3]+data_buff[i*16+4]+data_buff[i*16+5]+data_buff[i*16+6]-8500;
        data_return[4*i+1] = data_buff[i*16+0]+data_buff[i*16+7]+data_buff[i*16+8]+data_buff[i*16+13]+data_buff[i*16+14]+data_buff[i*16+15]-8500;
    ...
    QTextStream cout(stdout,  QIODevice::WriteOnly);//以下是调用python脚本处理数据
    PyObject * pModule = NULL;
    PyObject * pFunc = NULL;
    PyObject *pDict = NULL;
    PyObject *pReturn = NULL;
    PyObject *PyList  = PyList_New(320);//定义一个与数组等长的PyList对象数组
    PyObject *ArgList = PyTuple_New(1);//定义一个Tuple对象，Tuple对象的长度与Python函数参数个数一直，上面Python参数个数为1，所以这里给的长度为1

    pModule = PyImport_ImportModule("hello");
    if(!pModule){
        cout << "Why Can't open python file!\n" << endl;
    }
    pDict = PyModule_GetDict(pModule);
    pFunc = PyDict_GetItemString(pDict, "filter2");
    printf("\n");
    ...
    for(int i = 0; i < PyList_Size(PyList); i++)
    {
        PyList_SetItem(PyList,i, PyFloat_FromDouble(data_filter[i]));//给PyList对象的每个元素赋值
    }
    PyTuple_SetItem(ArgList, 0, PyList);//将PyList对象放入PyTuple对象中
    pReturn=PyObject_CallObject(pFunc, ArgList);//调用函数，完成传递
    ...
    for(int i = 0; i < SizeOfList; i++)
    {
        ...
        int result;
        PyArg_Parse(Item, "i", &result);//i表示转换成int型变量
        //PyArg_Parse(Item, "i", &data_return_python[i]);//i表示转换成int型变量
        //printf("%d ",result);
        data_return_python[i]=result;
        //cout << PyInt_AsLong(Item) <<" "; //输出元素
        //Py_DECREF(Item); //释放空间 new reference和和borrowed inference？
        //当函数的返回值是New reference时，需要对PyObject * 变量使用Py_DECREF(）
        //返回值是 Borrowed reference时，无需使用Py_DECREF(）
        //引用计数不变就是借用，引用计数加一就是引用
     }
    printf("\n");
    ...
    if(start_flag==1)
    thread_getdata_graph_plot(data_return_filter);
    else
    {
 ...
 * @retval 无
*/
void MyWidget::thread_getdata_graph_plot(int data_in[])//长度640
{
    static int i = 0;
    int a;
    int b;
    int data_in_python[640];
    ...
    PyObject *pDict = NULL;
    PyObject *pReturn = NULL;
    PyObject *PyList  = PyList_New(640);//定义一个与数组等长的PyList对象数组
    PyObject *ArgList = PyTuple_New(1);//定义一个Tuple对象，Tuple对象的长度与Python函数参数个数一直，上面Python参数个数为1，所以这里给的长度为1
    ...
    */
    ...
    for(i=0;i<160;i++)
    {
        a=a+data_in_python[4*i+0];
        b=b+data_in_python[4*i+1];
    }
    a=a/160;
    b=b/160;
    ...
    for(i=0;i<160;i++)
    {
        ...
        // 添加数据到graph
        QChart_coil->graph(0)->addData(key, data_in[4*i+0]);
        QChart_coil->graph(1)->addData(key, data_in[4*i+1]);
        QChart_hall->graph(0)->addData(key, data_in_python[4*i+0]-a);
        QChart_hall->graph(1)->addData(key, data_in_python[4*i+1]-b);
        qDebug()<<data_in_python[4*i+0];
        //qDebug()<<data_in[4*i+3];
        //记录当前时刻
        //曲线能动起来的关键在这里，设定x轴范围为最近8个时刻
        QChart_hall->xAxis->setRange(key, 45, Qt::AlignRight);
        QChart_coil->xAxis->setRange(key, 45, Qt::AlignRight);
...
//                PyRun_SimpleString("import sys");
//                PyRun_SimpleString("sys.path.append('D:/SYS ruanjian/python38/')"); // 设置绝对路径
